...
SERVICE_ACCOUNT_FILE = "your-private-google-service-key.json"  # replace with your JSON file name
...
def get_all_sitemap_urls(service, site_url):
    """Get all sitemap URLs from Google Search Console."""
    sitemaps = service.sitemaps().list(siteUrl=site_url).execute()
    if sitemaps.get('sitemap'):
        return [sitemap['path'] for sitemap in sitemaps['sitemap']]
    else:
        return []  # if no sitemaps, return empty list
...
def get_all_pages_from_all_sitemaps(sitemap_urls):
    """Get all page URLs from multiple sitemaps and merge them."""
    all_pages = []
    
    for sitemap_url in sitemap_urls:
        print(f"Processing sitemap: {sitemap_url}")
        pages = get_all_pages_from_sitemap(sitemap_url)
        print(f"Found {len(pages)} pages in {sitemap_url}")
        all_pages.extend(pages)
    
    # Remove duplicates while preserving order
    unique_pages = list(dict.fromkeys(all_pages))
    print(f"Total unique pages found across all sitemaps: {len(unique_pages)}")
...
def main():
    ...
    if INDEX_FROM_CSV:
        ...
        sitemap_urls = get_all_sitemap_urls(service, selected_site_url)
        print(f"Found {len(sitemap_urls)} sitemaps for {selected_site_url}:")
        for sitemap_url in sitemap_urls:
            print(f"  - {sitemap_url}")
        
        if sitemap_urls:
            pages_to_index = get_all_pages_from_all_sitemaps(sitemap_urls)
            print(f"All pages to index:")
            for page in pages_to_index[:10]:  # Show first 10 pages as preview
                print(f"  - {page}")
            if len(pages_to_index) > 10:
                print(f"  ... and {len(pages_to_index) - 10} more pages")
        else:
            print(f"No sitemaps found for {selected_site_url}.")
            return
